...
def find_hotels_bestdeal(chat_id: int, destination_id: str, check_in: datetime.date, check_out: datetime.date,
                         resultsize: int, num_photo: Optional[int]=None, bestdeal_params: Optional[dict]=None) -> None:
    ...
    payload = {
        ...
        "filters": {'availableFilter': 'SHOW_AVAILABLE_ONLY',
                              "price": {
                                  "max": bestdeal_params['max_price'],
                                  "min":  bestdeal_params['min_price']
                              }
                    }
    }
    ...
    if response:
        ...
        if 'data' in parsed:
            for elem in parsed['data']['propertySearch']['properties']:
                if bestdeal_params['min_dist'] <= float(elem['destinationInfo']['distanceFromDestination']['value']) <= bestdeal_params['max_dist']:
                    hotels.append(dict())
                    hotels[-1]['id'] = elem['id']
                    ...
                    if hotel_response:
                        ...
                        bad_flag = True
                if len(hotels) == resultsize:
                    break
            else:
                bot.send_message(chat_id, 'Отели, удовлетворяющие Вашим условиям:')
        else:
    ...
    if not bad_flag:
        with db:
            History(chat_id=chat_id, command='bestdeal', date=datetime.datetime.now(), hotels=hotels_db).save()
        for hotel in hotels:
